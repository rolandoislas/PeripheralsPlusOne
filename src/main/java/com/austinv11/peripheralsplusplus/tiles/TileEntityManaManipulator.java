...
public class TileEntityManaManipulator extends TileEntityInventory implements IPlusPlusPeripheral, ITickable, IManaPool,
    ...
    private boolean tryShootBurst() {
        ...
        if (System.currentTimeMillis() - lastAutoBurstTime >= 100 && reciever != null &&
            ...
            if (burst != null) {
                ...
                getWorld().spawnEntityInWorld((Entity) burst);
                burst.ping();
                worldObj.playSound(
                        null,
                        pos,
    ...
    private void updateReceiver() {
        ...
        if (receiver instanceof IManaReceiver && receiver.hasWorldObj() &&
    ...
    private ILensControl getLensControl(ItemStack lens) {
        if (lens != null && lens.getItem() instanceof ILensControl) {
    ...
    private Object[] getMana(Object[] arguments) throws LuaException {
        ...
        return new Object[]{((boolean) arguments[0]) ? manaHeld : worldObj.rand.nextInt(manaHeld + 1)};
    }
    ...
    public boolean bindTo(EntityPlayer player, ItemStack wand, BlockPos pos, EnumFacing side) {
        ...
        if (!axis.isVecInside(to))
            to = new Vec3d(
                    axis.minX + (axis.maxX - axis.minX) / 2,
                    axis.minY + (axis.maxY - axis.minY) / 2,
                    axis.minZ + (axis.maxZ - axis.minZ) / 2
            );
        ThreeDimensionalVector diff = new ThreeDimensionalVector(to.xCoord, to.yCoord, to.zCoord).subtract(
                new ThreeDimensionalVector(from.xCoord, from.yCoord, from.zCoord));
